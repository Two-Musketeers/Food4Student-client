...
import com.ilikeincest.food4student.screen.restaurant_owner.res_order.component.ResOrderCard
...
fun RestaurantOrderScreen(
    ...
    vm: RestaurantOrderViewModel = hiltViewModel()
) {
                    ...
                    itemsIndexed(currentList) { i, order ->
                        ResOrderCard(
                            ...
                            onNavigateToRestaurant = {
                                if (onNavigateToRestaurant == null) return@ResOrderCard
                                vm.fetchRestaurant(order.restaurantId, context, onSuccess = onNavigateToRestaurant)
                            },
                            onCancel = { vm.cancelOrder(order.id) },
...
private fun OrderPrev() {
    ScreenPreview { RestaurantOrderScreen({}) }
}